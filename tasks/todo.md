...
**Acceptance:**
- [ ] Every colour, space, radius, and type step is a token on `:root`, redefined under `prefers-color-scheme: dark` and overridable by a `data-theme` attribute
- [ ] Theme resolves OS → stored override → toggle, with no flash of wrong theme on load
- [ ] Toggle is a real `<button>` with `aria-pressed`, keyboard operable
